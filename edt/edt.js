...
async function getRawData(api_key, calendarId, bounds) {
    const calendarData = await calendar.events
        .list({
            key: api_key,
...
function parseEventName(name) {
    const regexp =
        /(\*\*EXAMEN\*\*)? ?([A-Z]{5}\d[A-Z]\d)?(?: \( ?(.+)\))?(?: - )?(.*?)(?: - ([A-Z]\. .*|Xfreee?) - )(.*)/gm;
    //const regexp =        /(\*\*EXAMEN\*\*)? ?([A-Z]{5}\d[A-Z]\d)?(?: \( ?(.+)\))?(?: - )?(.*?)(?: - ([A-Z]\.(\-[A-Z])?.*|Xfreee?) - )(.*)/gm;
